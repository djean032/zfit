module chem
   ...
   ! Module-level work arrays for ODE solvers (avoid repeated allocation)
   real(c_double) :: ode_rwork(102)
   integer(c_int) :: ode_iwork(25)
   ! Module-level work array for fcn callback
   real(c_double), allocatable :: cb_y_work(:)
   ! Precomputed constants for rhs_rates (avoid repeated divisions)
...
contains
   ...
   function solve_rates(y, t_in, t_out) result(y_ret)
      real(c_double), intent(in) :: t_in, t_out, y(6)
      integer(c_int) :: istate, neq
      real(c_double) :: t_ret, y_ret(6)
      neq = 5
      t_ret = t_in
      istate = 1
      y_ret = y
      call DLSODA(rhs_rates, neq, y_ret, t_ret, t_out, 1, &
                  1e-6_c_double, 1e-6_c_double, 1, istate, 0, ode_rwork, 102, ode_iwork, &
                  25, jdum, 2)
   end function solve_rates

   function solve_intensity(y, t_in, t_out) result(y_ret)
      real(c_double), intent(in) :: t_in, t_out, y(6)
      integer(c_int) :: istate, neq
      real(c_double) :: t_ret, y_ret(6)
      neq = 1
...
! Remove globals.
   function solve_system(z_positions, z_samples, times, initial_population, &
    ...
    real(c_double), intent(in) :: frq, initial_population(:), laser_intensities(:, :), spec_pars(8), times(:), z_positions(:), z_samples(:)
      ...
      ! Precompute constants for rhs_rates/rhs_intensity
      ...
      do pos_idx = 1, size(z_positions)
         ...
         do t_idx = 1, size(times)
            ...
            do sample_idx = 1, size(z_samples)
               ...
               current_population = solve_rates(current_population, 0.0_c_double, tout)
               pop(:, sample_idx) = current_population(1:5)
               current_intensity(2:6) = current_population(1:5)
               current_intensity = solve_intensity(current_intensity, 0.0_c_double, zout)
            end do
